...
const LabLogView = () => {
  ...
  const [parentStream, setParentStream] = useState(null);
  const [siblingLogs, setSiblingLogs] = useState([]);
  ...
  useEffect(() => {
    const fetchLog = async () => {
      try {
        ...
        // Fetch the lab log
        const logResponse = await client.queries.lab_logs({
          relativePath: `${filename}.mdx`
        });
        const logData = logResponse.data.lab_logs;
        setLog(logData);
        
        // Fetch all streams for sidebar
        ...
        // Fetch parent stream and siblings if it exists
        if (logData.parent_stream) {
          let parentStreamData;
          let parentPath;

          if (typeof logData.parent_stream === 'object') {
            parentStreamData = logData.parent_stream;
            parentPath = parentStreamData._sys.filename;
            setParentStream(parentStreamData);
          } else {
            parentPath = logData.parent_stream.replace('src/content/research-streams/', '').replace('.mdx', '');
            const streamResponse = await client.queries.research_streams({
              relativePath: `${parentPath}.mdx`
            });
            parentStreamData = streamResponse.data.research_streams;
            setParentStream(parentStreamData);
          }

          // Fetch siblings (logs in same stream)
          const logsRes = await client.queries.lab_logsConnection({ sort: 'date' });
          const allLogs = logsRes.data.lab_logsConnection.edges.map(e => e.node);
          
          const siblings = allLogs.filter(l => {
            if (!l.parent_stream) return false;
            const lParentPath = typeof l.parent_stream === 'object' 
              ? l.parent_stream._sys.filename 
              : l.parent_stream.replace('src/content/research-streams/', '').replace('.mdx', '');
            return lParentPath === parentPath && l._sys.filename !== filename;
          });
          
          setSiblingLogs(siblings);
        }
        
        setLoading(false);
      } catch (err) {
        console.error('Error fetching lab log:', err);
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center font-mono">
        <div className="text-cyan-500/70 text-center">
          <div className="mb-4 text-xs tracking-[0.2em] animate-pulse">RETRIEVING_DATA...</div>
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center font-mono">
        <div className="border border-red-500/30 bg-red-500/5 p-8 max-w-md w-full relative">
          ...
          <h2 className="text-xl text-white mb-4">LOG_NOT_FOUND</h2>
          <p className="text-red-400/70 text-sm mb-6">{error || 'The requested lab log could not be located.'}</p>
          <button
      ...
      <div className="relative z-10 container mx-auto px-6 py-12 flex flex-col lg:flex-row gap-12">
        ...
        <aside className="lg:w-64 flex-shrink-0 hidden lg:block">
          <div className="sticky top-24 space-y-8">
            {parentStream && (
              <div>
                <h3 className="text-xs font-mono !text-slate-500 mb-4 uppercase tracking-wider">// Parent_Stream</h3>
                <button
                  onClick={() => navigate(`/research/stream/${parentStream._sys.filename}`)}
                  className="text-sm !text-slate-500 hover:!text-cyan-400 font-medium mb-2 text-left block transition-colors"
                >
                  {parentStream.title}
                </button>
              </div>
              ...
              <div>
                ...
                <nav className="space-y-1">
                  {siblingLogs.map(l => (
                    <button
                      key={l._sys.filename}
                      onClick={() => navigate(`/research/log/${l._sys.filename}`)}
                    ...
                    >
                      <span className={`mr-2 ${getLogTypeColor(l.log_type)}`}>●</span>
                      {l.title}
                    </button>
                  ))}
                </nav>
          ...
          <article className="max-w-4xl">
            ...
            <header className="mb-12 border-b border-white/10 pb-12">
              <div className="flex flex-wrap items-center gap-4 mb-6 text-xs font-mono text-slate-500">
                <span className={`uppercase tracking-wider ${getLogTypeColor(log.log_type)}`}>[{log.log_type}]</span>
                <span>//</span>
                <span>{new Date(log.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              </div>
              ...
              {log.summary && (
                <div className="bg-[#0a0a0a] border-l-2 border-white/20 p-6 rounded-r-lg">
                  <h3 className="text-xs font-mono text-slate-500 mb-2 uppercase tracking-wider">Summary</h3>
            ...
            <div className="prose prose-invert max-w-none">
              <TinaMarkdown content={log.body} components={components} />
            </div>
          </article>
        </main>
      </div>
